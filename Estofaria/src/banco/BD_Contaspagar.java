...
public class BD_Contaspagar{
    ...
    public void Salvar(Contaspagar obj){
        sql = "insert into "+esquema+"."+tabela+"(nome,valor,data,status) values ("+"'"+obj.getNome()+"'"+",'"+obj.getValor()+"','"+(new SimpleDateFormat("yyyy-MM-dd").format(obj.getData()))+"',"+obj.getStatus()+")";
        //System.out.println(sql);
        kernel.getConexao().Save(sql);
    }

    public void Excluir(int id){
        sql = "delete from "+esquema+"."+tabela+" where contas_pagar_id = "+id;
        kernel.getConexao().Delete(sql);
    }

    public Contaspagar getContaspagarID(int id){
        try {
            sql = "select * from "+esquema+"."+tabela+" where contas_pagar_id = "+id;
    ...
    public void Atualizar(Contaspagar obj){
        sql = "update "+esquema+"."+tabela+" set nome = '"+obj.getNome()+"',valor = '"+obj.getValor()+"',data = '"+(new SimpleDateFormat("yyyy-MM-dd").format(obj.getData()))+"',status = "+obj.getStatus()+" where contas_pagar_id = "+obj.getContas_pagar_id();
       // System.out.println("SQL: "+sql);
        kernel.getConexao().Save(sql);
    }

    public Contaspagar[] getContaspagarNome(String nome) throws SQLException {
     sql = "select count(*) as total from "+esquema+"."+tabela+" where nome like '%"+nome+"%';";
            ...
            sql = "select * from "+esquema+"."+tabela+" where nome like '%"+nome+"%';";
          //  System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            int contador = 0;            
