...
public class Imprimir_Vendas {
    ...
    public void ImprimirVenda(int os) throws SQLException{
       ...
       if (pj != null) {
          ...
          g.drawString( ((obj_Vendas.getCliente()!=null)?obj_Vendas.getCliente().getNome().toUpperCase():" cliente sem cadastro ") , coluna, (linha+=10));
          g.drawString( "Fone: "+((obj_Vendas.getCliente()!=null)?((obj_Vendas.getCliente().getDdd_tel()==0)?"0":obj_Vendas.getCliente().getDdd_tel())+" "+((obj_Vendas.getCliente().getTelefone()==0)?"0":obj_Vendas.getCliente().getTelefone())+" "+((obj_Vendas.getCliente().getDdd_cel()==0)?"":obj_Vendas.getCliente().getDdd_cel())+""+((obj_Vendas.getCliente().getCelular()==0)?"":obj_Vendas.getCliente().getCelular()):"") , coluna, (linha+=10));
          g.drawString( ((obj_Vendas.getCliente()!=null)?((obj_Vendas.getCliente().getEndereco()==null)?"":obj_Vendas.getCliente().getEndereco())+" "+((obj_Vendas.getCliente().getNumero()==null)?"":obj_Vendas.getCliente().getNumero())+" "+((obj_Vendas.getCliente().getBairro()==null)?"":obj_Vendas.getCliente().getBairro()):""), coluna, (linha+=10));
          g.drawString( "------------------------------------------------" , coluna, (linha+=10));
          ...
          for(int i=0; i < vetor.length; i++){
              ...
              int tamanho_nome = obj_ItemProduto.getProdutos().getNome().length();
              
              g.drawString(obj_ItemProduto.getProdutos().getNome().substring(0,((tamanho_nome>15)?15:tamanho_nome) )+" \t\t\t\t\t\t\t\t "+
                            obj_ItemProduto.getQuantidade()+ " \t\t\t\t\t    "+
                            fmt.format((obj_Vendas.getParcelas()!=null)?obj_Vendas.getPreco_venda():((obj_ItemProduto.getProdutos().getIndice()!=0)?((((obj_ItemProduto.getProdutos().getIndice()!=0.0)?((obj_ItemProduto.getProdutos().getIndice())*(ouro.getNome())*(obj_ItemProduto.getProdutos().getPeso())):((obj_ItemProduto.getProdutos().getPreco_promocional()!=0.0)?obj_ItemProduto.getProdutos().getPreco_promocional():obj_ItemProduto.getProdutos().getPreco_venda())))*2):((obj_ItemProduto.getProdutos().getPreco_promocional()!=0.0)?obj_ItemProduto.getProdutos().getPreco_promocional():obj_ItemProduto.getProdutos().getPreco_venda()*obj_ItemProduto.getQuantidade()))) , coluna, (linha+=10));
