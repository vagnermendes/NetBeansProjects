...
public class BD_ClientesADM{
  ...
  //          System.out.println(sql);
    ...
    public ClienteADM getClienteAcesso(Funcionario obj) {
         try {
            sql = "select * from "+esquema+"."+tabela+"  c  where c.login = '"+obj.getLogin()+"' and c.senha = '"+obj.getSenha()+"' and c.esquema = '"+obj.getEsquema()+"'";
            //System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            while (rs.next()) {
                cliente.setCliente_id(Integer.parseInt(rs.getString("cliente_id")));
