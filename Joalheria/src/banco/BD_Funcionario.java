...
public class BD_Funcionario {
    ...
    public Funcionario getFuncionarioID(int id) throws SQLException{
            ...
            sql = "select *,b.nome as nivel_nome from "+esquema+"."+tabela+" a left join nivel b on a.nivel_id=b.nivel_id where funcionario_id = "+id;
            //System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            funcionario = new Funcionario();
            nivel       = new Nivel();
...
//                funcionario.setData_demissao((new SimpleDateFormat("yyyy-MM-dd")).parse(rs.getString("data_demissao"))); 
                ...
                Logger.getLogger(BD_Funcionario.class.getName()).log(Level.SEVERE, null, ex);
            }
            }
    ...
    public Funcionario getUsuario(Funcionario func) throws SQLException{
            ...
            sql = "select *,b.nome as nivel_nome from "+tabela+" a left join nivel b on a.nivel_id=b.nivel_id where a.login = '"+func.getLogin()+"' and a.senha='"+func.getSenha()+"'";
            //System.out.println(sql);
            rs = kernel.getConexao().Query(sql);            
    ...
    public int getFuncionarioIDNome(String obj){
        try {
            sql = "select * from "+tabela+" where upper(nome) = '"+obj+"'";
  //          System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            funcionario = new Funcionario();
            while (rs.next()) {
