...
public class BD_Produtos{
    ...
    public Produtos[] getProdutosNome(String nome) throws SQLException {
            sql = "select count(*) as total from "+tabela+" where  nome like '%"+nome.toUpperCase()+"%' or codigo_barra='"+ nome.toUpperCase()+"'";            
            //System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            int total=0;
            while (rs.next()) {
            ...
            sql = "select * from "+tabela+" where nome like '%"+nome.toUpperCase()+"%' or codigo_barra='"+ nome.toUpperCase()+"';";            
            //System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            int contador = 0;
            while (rs.next()) {
    ...
    public Produtos getProdutoCodigoBarras(String codigo) throws SQLException {
        sql = "select * from "+tabela+" where codigo_barra = '"+codigo+"'";
            //System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            obj_Produtos = new Produtos();
            while (rs.next()) {
