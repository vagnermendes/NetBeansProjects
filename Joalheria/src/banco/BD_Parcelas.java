...
public class BD_Parcelas{
    ...
    public Parcelas[] getParcelas() throws SQLException{
            ...
            sql = "select * from "+tabela+" order by parcelas_id asc;";
            rs = kernel.getConexao().Query(sql);
            int contador = 0;
            while (rs.next()) {
    ...
    public void Salvar(Parcelas Parcelas){
        sql = "insert into "+tabela+"(nome) values ("+"'"+Parcelas.getNome()+"'"+ ")";
        //////System.out.println(sql);
        kernel.getConexao().Save(sql);
    }

    public void Excluir(int id){
        sql = "delete from "+tabela+" where Parcelas_id = "+id;
        kernel.getConexao().Delete(sql);
    }

    public Parcelas getParcelasID(int id){
        try {
            sql = "select * from "+tabela+" where Parcelas_id = "+id;
    ...
    public void Atualizar(Parcelas obj){
        sql = "update "+tabela+" set nome = '"+obj.getNome()+"' where Parcelas_id = "+obj.getParcelas_id();
