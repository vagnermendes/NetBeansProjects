...
public class BD_Cartoes{
    ...
    public Cartoes[] getCartoes() throws SQLException{
            ...
            sql = "select * from "+tabela+" order by cartoes_id asc;";
            rs = kernel.getConexao().Query(sql);
            int contador = 0;
            while (rs.next()) {
    ...
    public void Salvar(Cartoes Cartoes){
        sql = "insert into "+tabela+"(nome) values ("+"'"+Cartoes.getNome()+"'"+ ")";
        //////System.out.println(sql);
        kernel.getConexao().Save(sql);
    }

    public void Excluir(int id){
        sql = "delete from "+tabela+" where Cartoes_id = "+id;
        kernel.getConexao().Delete(sql);
    }

    public Cartoes getCartoesID(int id){
        try {
            sql = "select * from "+tabela+" where Cartoes_id = "+id;
  ...
  //          System.out.println(sql);
    ...
    public void Atualizar(Cartoes obj){
        sql = "update "+tabela+" set nome = '"+obj.getNome()+"' where Cartoes_id = "+obj.getCartoes_id();
