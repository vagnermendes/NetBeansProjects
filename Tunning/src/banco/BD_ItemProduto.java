...
public class BD_ItemProduto {
    ...
    public ItemProduto[] getItemProdutoVenda(int venda_id) throws SQLException, ParseException{
            ...
            sql = "select *,b.nome as produto_nome,d.nome as cartoes,f.nome as parcelas from "+esquema+"."+tabela+" a left join produtos b on a.produtos_id=b.produtos_id "
                    +" left join vendas c on a.vendas_id=c.vendas_id "
                    +" left join cartoes d on c.cartoes_id=d.cartoes_id "
                    +" left join parcelas f on c.parcelas_id=f.parcelas_id "
    ...
    public void Salvar(ItemProduto ItemProduto) throws SQLException{
        ...
        sql = "select quantidade from produtos where produtos_id = "+ItemProduto.getProdutos().getProdutos_id();        
        //System.out.println(sql);
        rs = kernel.getConexao().Query(sql);
        double quantidade=0;
        double total=0;
        ...
        sql = "update produtos set quantidade = "+total+" where produtos_id = "+ItemProduto.getProdutos().getProdutos_id();        
        //System.out.println(sql);
