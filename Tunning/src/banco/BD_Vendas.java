...
public class BD_Vendas {
    ...
    public void Salvar(Vendas Vendas){        
        sql = "insert into "+esquema+"."+tabela+"(data,hora,preco,preco_venda,entrada,restante,desconto,cliente_id) values ('"+(new SimpleDateFormat("yyyy-MM-dd").format(Vendas.getData()))+"','"+(new SimpleDateFormat("HH:mm").format(Vendas.getHora()))+"',"+Vendas.getPreco()+","+Vendas.getPreco_venda()+","+Vendas.getEntrada()+","+Vendas.getRestante()+","+Vendas.getDesconto()+","+Vendas.getCliente().getCliente_id()+")";
    ...
    public void Excluir(int id) throws SQLException{
        ...
        sql = "select * from item_produto where vendas_id = "+id;     
        rs = kernel.getConexao().Query(sql);
        //System.out.println(sql);
        while (rs.next()) {
            ...
            sql = "select quantidade from produtos where produtos_id = "+obj_ItemProduto.getProdutos().getProdutos_id();        
            //System.out.println(sql);
            ...
            sql = "update produtos set quantidade = "+total+" where produtos_id = "+obj_ItemProduto.getProdutos().getProdutos_id();        
            //System.out.println(sql);
            kernel.getConexao().Save(sql);
        ...
        sql = "delete from item_produto where vendas_id = "+id;
        //System.out.println(sql);
    ...
    public Vendas getVendasID(int id) throws ParseException{
        try {
            sql = "select *,d.nome as cartoes,f.nome as parcelas from "+esquema+"."+tabela+" a left join cliente b on a.cliente_id=b.cliente_id "
                    +" left join cartoes d on d.cartoes_id=a.cartoes_id "
                    +" left join parcelas f on f.parcelas_id=a.parcelas_id "
    ...
    public Vendas[] getRelatorio(int funcionario_id, String data1, String data2) throws SQLException, ParseException {
        ...
        rs = kernel.getConexao().Query(sql);
            ...
            sql = "select *,b.nome as funcionario_nome,c.nome as cliente_nome from "+esquema+"."+tabela+" a left join funcionario b on a.funcionario_id=b.funcionario_id left join cliente c on a.cliente_id = c.cliente_id where a.data between '"+data1+"' and '"+data2+"' "+((funcionario_id!=0)?"and a.funcionario_id="+funcionario_id:"");
           // System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            int contador = 0;
            while (rs.next()) {
