...
public class BD_Categoria{
    ...
    public void Salvar(Categoria categoria){
        sql = "insert into "+tabela+"(nome) values ("+"'"+categoria.getNome()+"'"+ ")";
        //////System.out.println(sql);
        kernel.getConexao().Save(sql);
    }

    public void Excluir(int id){
        sql = "delete from "+tabela+" where categoria_id = "+id;
        kernel.getConexao().Delete(sql);
    }

    public Categoria getCategoriaID(int id){
        try {
            sql = "select * from "+tabela+" where categoria_id = "+id;
    ...
    public int getCategoriaIDNome(String obj){
        try {
            sql = "select * from "+tabela+" where nome = '"+obj.toLowerCase()+"'";
    //        System.out.println(sql);
            rs = kernel.getConexao().Query(sql);
            obj_Categoria = new Categoria();
            while (rs.next()) {
    ...
    public void Atualizar(Categoria obj){
        sql = "update "+tabela+" set nome = '"+obj.getNome()+"' where categoria_id = "+obj.getCategoria_id();
